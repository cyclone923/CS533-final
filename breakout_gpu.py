    ...
    def train(self):
        ...
        while True:
            if trainExamples - i_epoch * capacity / 10 >= capacity / 10:
                ...
                torch.save(self.net.state_dict(), 'netWeight/breakoutTS/cuda/' + str(i_epoch) + '.pth')
                self.net.cpu()
                torch.save(self.net.state_dict(), 'netWeight/breakoutTS/cpu/' + str(i_epoch) + '.pth')
                self.net.cuda()
                i_epoch += 1
                if i_epoch == 100:
                    break
            ...
            while True:
                ...
                if done:
                    ...
                    print("Net3 Score in %d episode: %d" % (i_episode, eval))
                    print("")
                    i_episode += 1
...
if __name__ == "__main__":
    agent = Agent("BreakoutDeterministic-v4", gpu=True)
    agent.train()

...
class Agent(object):
    ...
    def update(self, sample):
        ...
        diff = np.zeros(shape=(batch_size, 10))
        indicate = np.ones(shape=(batch_size, 10))
    ...
    def train(self):
        ...
        while True:
            if trainExamples - i_epoch * capacity >= capacity:
                ...
                torch.save(self.net.state_dict(), 'netWeight/amidar/cuda/' + str(i_epoch) + 'beta1.pth')
                self.net.cpu()
                torch.save(self.net.state_dict(), 'netWeight/amidar/cpu/' + str(i_epoch) + 'beta1.pth')
                self.net.cuda()
                i_epoch += 1
                if i_epoch == 100:
                    break
...
if __name__ == "__main__":
    agent = Agent("AmidarNoFrameskip-v0")
    agent.train()

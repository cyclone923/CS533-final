...
class Net(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(in_features=3584, out_features=256)
...
def refresh(old,obs):
    new = np.empty(shape=(1, 4, 125, 80),dtype=np.uint8)
    new[0][3] = old[0][2]
    new[0][2] = old[0][1]
    new[0][1] = old[0][0]
    new[0][0] = obs
...
class Agent(object):
    ...
    def update(self, sample):
        ...
        batch_x = np.empty(shape=(batch_size, 4, 125, 80),dtype=np.uint8)
        batch_xNew = np.empty(shape=(batch_size, 4, 125, 80),dtype=np.uint8)
    ...
    def train(self):
        ...
        while True:
            if trainExamples - i_epoch * capacity/10 >= capacity/10:
                ...
                self.net.cpu()
                torch.save(self.net.state_dict(), 'netWeight/amidar/cpu/' + str(i_epoch) + '.pth')
                self.net.cuda()
                i_epoch += 1
                if i_epoch == 100:
                    break
            ...
            frames = np.empty(shape=(1, 4, 125, 80),dtype=np.uint8)  # batch_size,channels,x,y
            frames[0][0] = obs
            frames[0][1] = obs
            frames[0][2] = obs
            frames[0][3] = obs
...
if __name__ == "__main__":
    agent = Agent("AmidarDeterministic-v4",gpu=True)
    agent.train()
